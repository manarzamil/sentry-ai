...
<p align="center">
  <img src="assets/3d-model/sentry-ai-helmet-render-front.png" alt="Sentry AI smart safety helmet, front elevation of the 3D product render" width="540">
</p>

<p align="center">
  <b>Figure 1 &mdash; Sentry AI helmet, front elevation (3D concept render).</b><br>
  Shows the intended physical integration of the electronics into a standard industrial hard hat: the controller enclosure and the GPS module sit on the crown for clear sky view, with the cable dropping to the ESP32 board and the 18650 cell carried under the shell. Nothing in the assembly obstructs the visor or the chin strap.
</p>

<p align="center">
  <img src="assets/3d-model/sentry-ai-helmet-render-angle.png" alt="Sentry AI smart safety helmet, three-quarter top view of the 3D product render" width="420">
</p>

<p align="center">
  <b>Figure 2 &mdash; Three-quarter top view.</b><br>
  Shows the footprint of the enclosure relative to the crown and the retention strap, and the aerial placement of the GPS module &mdash; the detail that determines whether a fix can be acquired while the helmet is being worn.
</p>

> **These are design renders of the intended product, not photographs of the assembled prototype.** The interactive model is available at [studio.tripo3d.ai](https://studio.tripo3d.ai/3d-model/c44f07f1-ff50-4713-a6bd-9200887e1228?invite_code=KHUC1Y). Photographs of the built hardware will be added once the prototype is assembled. Render notes are in [`assets/3d-model/README.md`](assets/3d-model/README.md).

### Dashboard interface

> **Placeholder &mdash; no dashboard screenshot exists yet.**
>
> The dashboard is generated inside the firmware and served by the ESP32 over its own Access Point, so capturing it requires powered hardware. **No screenshot has been mocked up, simulated or otherwise invented for this README.** The real capture will be committed to [`assets/screenshots/`](assets/screenshots/) after hardware testing, and this block will be replaced with the image.
>
> Until then the interface is specified field by field in [&sect;13 Dashboard and User Experience](#13-dashboard-and-user-experience) and [`docs/api/http-api.md`](docs/api/http-api.md).

### Demonstration clip

> **Placeholder &mdash; no footage recorded yet.** A 15&ndash;30 second clip showing a triggered fall, the audible alarm and the resulting dashboard alert will be added to [`assets/demo/`](assets/demo/). See [Demo](#demo).

---

## System Demonstration

The end-to-end path from a physical fall to a located, alarmed, supervisor-visible incident. Every stage below is implemented in the firmware; the timings that connect them have **not** been measured yet, which is the subject of [&sect;15](#15-testing-and-validation).

```mermaid
flowchart LR
    A["<b>Helmet</b><br/>MPU-6050 + NEO-6M<br/>worn, master switch on"]
    B["<b>Fall Detection</b><br/>freefall &rarr; impact &rarr; stillness<br/>in sequence, time-bounded"]
    C["<b>ESP32 Processing</b><br/>latch alert, log event,<br/>write to NVS, cluster zone"]
    D["<b>Local Dashboard</b><br/>self-hosted over SoftAP"]
    E["<b>Alarm</b><br/>buzzer pattern 0<br/>non-blocking"]
    F["<b>GPS Location</b><br/>live fix, or last known<br/>fix with age reported"]
    A --> B --> C --> D --> E --> F
```

| # | Stage | What happens | Implemented in | Observable evidence |
|---|---|---|---|---|
| 1 | **Helmet** | The worker closes the master switch; the IMU and GPS are sampled in a cooperative loop at roughly 50 Hz, and a worn-state heuristic confirms the helmet is on a head rather than on a bench | `setup()`, `loop()` | Startup beep, and `[BOOT]` / `[OK]` lines on the serial monitor |
| 2 | **Fall Detection** | A four-state machine requires freefall below 4.9 m/s&sup2;, an impact above 24.5 m/s&sup2; within 500 ms, and 600 ms of post-impact stillness, in that order, before anything is raised | `detectFall()` | `[FALL]` state transitions on the serial monitor |
| 3 | **ESP32 Processing** | The alert is latched so it cannot be missed, the fall counter increments, the event and its coordinates are written to non-volatile storage, and hazard-zone clustering is updated | `triggerFall()`, `saveEvent()`, `updateZone()` | `[FALL] CONFIRMED` on serial; the incident survives a power cycle |
| 4 | **Local Dashboard** | A supervisor already joined to the helmet Access Point sees the state change on the next refresh cycle and clears it with Reset Alert once they have responded | `handleRoot()`, `handleData()` | Red alert state on the dashboard; `fall_alert` true in the `/data` payload |
| 5 | **Alarm** | The buzzer plays the fall pattern, generated by elapsed-time comparison rather than blocking delays, so detection continues while the alarm sounds | `runBuzzer()` | Audible pattern, distinct from the SOS and fatigue patterns |
| 6 | **GPS Location** | The incident is geotagged with the live fix, or with the last valid fix plus the number of seconds since it was lost, so a responder is never shown a stale coordinate that looks live | `readGPS()` | Coordinates on the dashboard, or an explicit cached / no-fix state |

Two honest caveats about this flow. Dashboard visibility is bounded by the 5-second HTML meta refresh, so up to five seconds of *display* latency is expected by design and is not a detection delay. And the alert reaches only devices associated with that individual helmet Access Point &mdash; there is no long-range uplink, which is the highest-priority gap in [&sect;17](#17-future-improvements).
...
**Visual material:** [Product Preview](#product-preview) &middot; [System Demonstration](#system-demonstration) &middot; [Test Matrix](#test-matrix) &middot; [Demo](#demo)
...
The framework below is in place, but **no value in the two right-hand columns has been filled in and none will be estimated.** Each cell stays `TODO` until a real measurement is recorded.

| Test Case | Expected Behavior | Measured Response Time | Result |
|---|---|---|---|
| Simulated fall | Fall detected and alarm triggered | TODO | TODO |
| Strong impact | Impact detected and alarm triggered | TODO | TODO |
| Normal movement | No false fall alert | TODO | TODO |
| Running/movement | No false fall alert | TODO | TODO |
| GPS acquisition | Location displayed when fix is available | TODO | TODO |

#### Test case definitions

| ID | Test case | Procedure | Pass criterion |
|---|---|---|---|
| TC-01 | Simulated fall | Release the helmet from roughly 1.5 m onto crash matting, varying orientation between crown-first, side and rear. Let it come to rest and stay still. | `[FALL] CONFIRMED` on serial, buzzer pattern 0 audible, dashboard shows the latched alert |
| TC-02 | Strong impact | Deliver a sharp impact to the shell with no preceding freefall, for example a firm strike against a padded beam while the helmet is held. | Record the actual behaviour. See the note below &mdash; a non-trigger here may be correct by design |
| TC-03 | Normal movement | Wear the helmet for five continuous minutes of ordinary work: walking, bending, kneeling, sitting down heavily, setting the helmet down on a bench, and holding a vibrating tool. | No fall alert raised at any point |
| TC-04 | Running/movement | Run for 60 s, climb and descend a ladder, jump down from a low step, and shake the head vigorously. | No fall alert raised at any point |
| TC-05 | GPS acquisition | Cold-start outdoors with clear sky view and record time to first fix. Then walk indoors and observe the transition to the cached state. | Live coordinates shown while the fix is valid; cached position labelled as cached with `gps_lost_s` increasing |

> **Note on TC-02.** The state machine requires freefall, then impact, then stillness, in that order. An impact with no preceding freefall may therefore **not** raise an alarm, and that would be correct behaviour rather than a failure. Record what actually happens. If the result is a non-trigger, report it as a characteristic of the algorithm; do not retune thresholds to make the test pass and then present the retuned result as the original design.

### What "response time" means here

A single number is not enough, because the delay a worker experiences and the delay a supervisor experiences are different things. Five timestamps must be captured per trial.

| Marker | Event | How to capture |
|---|---|---|
| **T0** | The physical event occurs &mdash; the moment of impact | Video frame in which the helmet strikes the surface |
| **T1** | The ESP32 confirms the fall | Video frame of the `[FALL] CONFIRMED` line appearing in the serial monitor, or its `millis()` value |
| **T2** | The buzzer activates | First frame of audible alarm on the video audio track |
| **T3** | The dashboard reflects the alert | Frame in which the supervisor device switches to the alert state |
| **T4** | GPS coordinates are available for the incident, **only if a valid fix exists** | Frame in which coordinates are shown for that event; otherwise record `no fix` |

| Derived interval | Meaning |
|---|---|
| T1 &minus; T0 | Detection latency. Bounded below by the 600 ms stillness requirement, so a value under 600 ms would indicate a measurement error |
| T2 &minus; T1 | Alarm actuation latency |
| T2 &minus; T0 | **Total time to audible alarm** &mdash; the headline figure for a worker nearby |
| T3 &minus; T0 | Time to supervisor visibility. Includes up to 5 s of HTML meta-refresh delay, which is a property of the display design and not of detection. Report the two separately |
| T4 &minus; T0 | Time to a usable location for the incident, or `no fix` if none was ever acquired |

#### Capture methods

**Method A &mdash; single-camera video, 60 fps.** Frame the helmet, the serial monitor and the supervisor device in one shot, record with audio, then count frames. Resolution is about 17 ms per frame, which is more than sufficient here, and one clip yields T0 through T4 on a common clock. This is the recommended method because it needs no firmware changes.

**Method B &mdash; firmware timestamps.** Print `millis()` at state entry, at confirmation and at buzzer start, and read the serial log at 115200 baud. This gives precise internal timing but cannot see T0, so it must still be anchored to an external reference for the total figure.

### Test procedure

1. **Prepare.** Fully charge the cell, flash the firmware version under test and note its commit hash, open the serial monitor at 115200 baud and log it to a file, set the camera to 60 fps, and lay out crash matting.
2. **Establish a baseline.** Close the master switch, confirm the startup beep and the `[OK]` initialisation lines, join the helmet Access Point, load the dashboard, and note the GPS state before starting.
3. **Run 10 trials per test case.** Repeat each case at least ten times; more is better for the movement cases, where false positives are rare events and a small sample proves little.
4. **Reset between trials.** Clear the latched alert with Reset Alert, confirm the dashboard has returned to the safe state and the buzzer is silent, then wait a few seconds before the next trial.
5. **Record every trial**, including the ones that fail, misfire or behave oddly. Discarding inconvenient trials is what makes a test matrix worthless.
6. **Log the conditions.** Indoor or outdoor, surface, drop height, orientation, GPS state, battery voltage and ambient vibration.
7. **Do not change thresholds mid-series.** If any constant is altered, start a new series and record the new firmware version against it.
8. **Safety.** Never test a fall while wearing the helmet, and never from height. Drop the helmet alone or on a weighted dummy, onto matting. This is a prototype and is not life-safety certified.

#### Recording sheet

Log one row per trial. A plain CSV is enough, and it keeps the raw data auditable behind whatever summary is published.

```csv
trial,test_case,firmware,T0_ms,T1_ms,T2_ms,T3_ms,T4_ms,detected,gps_state,conditions,notes
1,TC-01,,,,,,,,,,
2,TC-01,,,,,,,,,,
3,TC-01,,,,,,,,,,
```

### Results summary &mdash; to be completed

| Test case | Trials | Detected | False positives | False negatives | Avg T2&minus;T0 (ms) | Min (ms) | Max (ms) |
|---|---|---|---|---|---|---|---|
| TC-01 Simulated fall | TODO | TODO | TODO | TODO | TODO | TODO | TODO |
| TC-02 Strong impact | TODO | TODO | TODO | TODO | TODO | TODO | TODO |
| TC-03 Normal movement | TODO | TODO | TODO | TODO | TODO | TODO | TODO |
| TC-04 Running/movement | TODO | TODO | TODO | TODO | TODO | TODO | TODO |
| TC-05 GPS acquisition | TODO | TODO | TODO | TODO | TODO | TODO | TODO |

When the real measurements are available, this section is to report, per test case: the number of trials, the average, minimum and maximum response time, the detection result, and any false positives or false negatives observed &mdash; alongside the raw CSV so the summary can be checked against it.

_TODO: execute the protocol and the matrix above and publish the resulting figures. Every value above remains TODO until it has actually been measured; nothing here is to be filled in by estimation._
...
> **Placeholder &mdash; a 15&ndash;30 second demonstration clip will be added here once it has been recorded.** Nothing has been staged, simulated or borrowed in its place.

Shot list for the clip, in order, so that cause and effect are visible in a single continuous take:

| Seconds | Shot | Purpose |
|---|---|---|
| 0&ndash;4 | Helmet powered and monitoring, dashboard in the safe state | Establishes the normal baseline |
| 4&ndash;8 | The fall, in frame, onto matting | Shows the physical event being detected, not a button press |
| 8&ndash;14 | Buzzer audible, dashboard flipping to the red alert state | Demonstrates local alarm and supervisor visibility together |
| 14&ndash;22 | Dashboard held on screen: fall status, session count, GPS state and coordinates | Shows the incident record and the geotag |
| 22&ndash;30 | Reset Alert pressed, system returning to the safe state | Shows the operator acknowledgement path |

Once the file exists at [`assets/demo/`](assets/demo/), embed it here with:

```markdown
![Sentry AI fall detection demonstration](assets/demo/sentry-ai-demo.gif)
```

### Visual assets: what exists and what is pending

| Asset | Location | Status |
|---|---|---|
| 3D product renders, front and three-quarter | [`assets/3d-model/`](assets/3d-model/) | **Available** &mdash; see [Product Preview](#product-preview) |
| Photographs of the assembled helmet | `assets/3d-model/` | Pending assembly |
| Dashboard in the safe state | `assets/screenshots/` | Pending hardware testing |
| Dashboard during an active fall alert | `assets/screenshots/` | Pending hardware testing |
| Serial capture of the `[FALL] CONFIRMED` sequence | `assets/screenshots/` | Pending hardware testing |
| Raw `/data` JSON payload | `assets/screenshots/` | Pending hardware testing |
| Demonstration clip, 15&ndash;30 s | [`assets/demo/`](assets/demo/) | Pending recording |
| Measured performance figures | [&sect;15](#15-testing-and-validation) | Pending measurement |

### Measured results

_TODO: no measured results exist yet._ The test matrix, the response-time definitions and the procedure needed to produce them are set out in [&sect;15](#15-testing-and-validation). This section will carry the summary figures &mdash; trials, average, minimum and maximum response time, detection outcome and any false positives or negatives &mdash; once the trials have been run. Until then it reports the absence rather than filling it with plausible numbers.
...
├── assets/
│   ├── 3d-model/
│   │   ├── sentry-ai-helmet-render-front.png  Product render, front elevation
│   │   ├── sentry-ai-helmet-render-angle.png  Product render, three-quarter view
│   │   └── README.md                          Render notes and source model
│   ├── screenshots/
│   │   └── README.md                          Pending dashboard captures
│   └── demo/
│       └── README.md                          Pending demonstration clip
├── docs/
│   ├── architecture/
